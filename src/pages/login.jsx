export default function LoginPage(){
    return(
        <div className="w-fill h-screen bg-red-100 flex flex-col justify-evenly items-center">
            <h1 className="font-bold text-[30px] text-blue-700">Crystel Beauty Clear</h1>
            <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Totam, voluptas?</p>
            <button className="bg-blue-500 text-white font-bold py-2 px-4 rounded">Login</button>
        </div>
    )
}